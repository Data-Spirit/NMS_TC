...
> **Dernière mise à jour de ce prompt : 2026-09-09** — ajout de l'auto-versionnage, de la section « LES FICHIERS FOURNIS », de la règle de fraîcheur étendue (hash, fraîcheur de `companion_html_guide`), de la restriction de `companion_html_guide` à la fourniture explicite, de la section « PROTECTION DES MODIFICATIONS DE CE PROMPT », de la section « ATTENDRE LE FEU VERT AVANT DE TRADUIRE », d'une « Note de portée » sur la langue source, d'une section « VÉRIFICATIONS TECHNIQUES RECOMMANDÉES », d'une colonne de traçabilité `companion_html_guide` dans l'historique, correction des noms de fichiers obsolètes (`translation_prompt_*` → `prompt_translation_*`), et réorganisation des sections pour suivre le même enchaînement que `prompt_translation_HTML_GUIDE_LANG.md`. Cette ligne doit être mise à jour à chaque modification validée de ce prompt (voir section « PROTECTION DES MODIFICATIONS DE CE PROMPT ») — si elle semble absente ou visiblement ancienne par rapport au contenu, traiter ce prompt comme potentiellement obsolète et le signaler à la personne avant de l'utiliser.
