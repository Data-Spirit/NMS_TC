...
1. [🏷️ C'est quoi, un badge Shields.io ?](#sec-1)
2. [🔍 Pourquoi passer par un Endpoint JSON plutôt qu'un badge statique ?](#sec-2)
3. [🖼️ Piège courant : un "SVG" peut être un PNG déguisé](#sec-3)
4. [🖌️ Cas A — Badge avec logo SVG custom (`logoSvg`)](#sec-4)
5. [🗂️ Cas B — Badge avec logo de la bibliothèque simple-icons (`namedLogo`)](#sec-5)
6. [⚖️ `namedLogo` vs `logoSvg` : lequel utiliser ?](#sec-6)
7. [🛠️ Générer correctement la chaîne `logoSvg`](#sec-7)
8. [🌐 Héberger le JSON et construire l'URL du badge](#sec-8)
9. [🎯 Exemple concret — badge Nexus Mods (NMS_TC)](#sec-9)
10. [🎭 Variantes stylistique du Badge](#sec-10)
11. [📚 Sources](#sec-11)
...
<a id="sec-1"></a>
## 1. 🏷️ C'est quoi, un badge Shields.io ?

Un badge est ce petit rectangle coloré qu'on voit en haut des README GitHub
(`build: passing`, `license: MIT`, etc.).

[Shields.io](https://shields.io) est le service qui génère ces images à la
volée : tu lui donnes une URL, il te renvoie un `.svg` prêt à afficher.
...
de logos de marques/outils/services.

Mais si le service que tu veux représenter (dans notre cas Nexus Mods) n'y
figure pas, `namedLogo` ne suffit plus.
...
> [!IMPORTANT]
> C'est pourquoi il est toujours préférable d'utiliser un vrai SVG
...
<a id="sec-4"></a>
## 4. 🖌️ Cas A — Badge avec logo SVG custom (`logoSvg`)
...
<a id="sec-5"></a>
## 5. 🗂️ Cas B — Badge avec logo de la bibliothèque simple-icons (`namedLogo`)
...
Seuls les champs suivants sont spécifiques à un logo de bibliothèque
plutôt qu'à un `logoSvg` custom :
...
<a id="sec-6"></a>
## 6. ⚖️ `namedLogo` vs `logoSvg` : lequel utiliser ?
...
<a id="sec-7"></a>
## 7. 🛠️ Générer correctement la chaîne `logoSvg`
...
<details>
<summary><b>🟩 7b. Avec Node.js (Windows/Mac/Linux, une seule commande)</b></summary>
...
> [!WARNING]
> **Deux niveaux de cache à connaître :**
> - `raw.githubusercontent.com` est mis en cache environ 5 minutes côté GitHub.
> - Le champ `cacheSeconds` du JSON (voir §4.2) a un défaut et un minimum de 300 secondes (5 min) côté Shields — une valeur plus basse est ignorée.
>
...
Cas réel utilisé dans ce projet :\
Le logo Nexus Mods n'existe pas dans simple-icons, on a donc recréé son SVG vectoriel à la main 
(plutôt que de réutiliser un PNG encodé en base64 trouvé ailleurs) et on l'a intégré en `logoSvg` :

<ins>[Fichier :](https://github.com/Data-Spirit/NMS_TC/blob/main/assets/nexusmods_logo.svg)</ins> `nexusmods_logo.svg` :⤵️
...
```
<ins>[Fichier :](https://github.com/Data-Spirit/NMS_TC/blob/main/assets/nexusmods_badge.json)</ins> `nexusmods_badge.json` :⤵️
...
>
> Ces couleurs correspondent aux couleurs Nexus Mods retenues pour le badge.
...
<a id="sec-10"></a>
## 10. 🎭 Variantes stylistique du Badge
